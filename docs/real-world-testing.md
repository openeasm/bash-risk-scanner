...
当前基线为 115 条离线语料：114 条完全匹配，类别级 precision、recall 和
...
暂存及私钥位置清单也已转入 validation。当前冻结样本改为 Atomic T1685.004 的
`auditctl -e 0`；扫描器能识别防御规避类别和泛化安全控制关闭，但尚未识别高
置信度的 `defense.audit-disable` 具体语义。发布门禁继续要求整体
precision 95%、recall 90%、regression 完全匹配，且
`maximum.forbiddenFindingCount` 为 0。下一轮应区分 `auditctl -e 0` 与启用、
锁定、状态查询、其他参数、帮助、文本和函数遮蔽。

这些数字只用于版本间回归对比。门槛应随着更多授权真实语料持续校准，不能从
158 个单元测试或当前小规模公开语料外推生产环境准确率。
