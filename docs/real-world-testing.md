...
当前基线为 120 条离线语料：119 条完全匹配，类别级 precision 为 100%、
...
- Atomic Red Team T1105 的 SFTP 上传现在关联静态远端会话与 here-string 中
  ...
  普通连接、文本和函数遮蔽均不命中。
- Atomic Red Team T1685 的 journald 服务停止现在解析 systemctl 动作和精确
  单元名；启动、重启、启用、解除 mask、状态查询、其他单元、帮助、文本和
  函数遮蔽均不命中。
