...
当前公开语料包括 MIT 许可的 nvm、Atomic Red Team、pipx、pnpm self-installer
与 node-gyp，以及 BSD-2-Clause 许可的 Homebrew；CI 使用仓库内快照，不联网
下载，也不会执行样本。冻结 test 分层不用于本轮规则调参，报告会如实保留 FN。
