...
变量 Python、Keychain 与 T1543.001 emond 持久化步骤已转为 validation 回归；
冻结 test 分层当前使用 T1048.003 `python3 -m http.server` 数据外传步骤。本轮
不针对新 test 调参，报告会如实保留 FP、FN 及 finding 约束错误。
