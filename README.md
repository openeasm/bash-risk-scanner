...
变量 Python、Keychain、emond、Python HTTP server 与 T1686 `iptables -F`
步骤已转为 validation 回归；冻结 test 分层当前使用 T1486 的变量定位 GPG 后对
文件进行对称加密的步骤。本轮不针对新 test 调参，报告会如实保留 FP、FN 及
finding 约束错误。
