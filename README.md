...
变量 Python、Keychain、emond、Python HTTP server、iptables flush 与变量定位
GPG 后对称加密的步骤已转为 validation 回归；冻结 test 分层当前使用 T1490
禁用 macOS Time Machine 的步骤。本轮不针对新 test 调参，报告会如实保留 FP、
FN 及 finding 约束错误。
