...
export const PYTHON_RULES: LanguageRule[] = [
  ...
  call({ id: "python.network", category: "network_egress", title: "Creates an outbound network connection", severity: "medium", confidence: "medium", message: "Uses an HTTP, URL, socket, SSH, telnet, FTP, or proven object-storage client.", pattern: /^(?:requests\.(?:Session\.)?(?:get|post|put|patch|delete|head|options|request)|aiohttp\.ClientSession\.(?:get|post|put|patch|delete|head|options|request|ws_connect)|httpx\.(?:(?:Async)?Client\.)?(?:get|post|put|patch|delete|head|options|request|stream)|urllib\.request\.(?:urlopen|urlretrieve)|socket\.(?:socket|create_connection)|paramiko\.\w+|telnetlib3\.open_connection|ftplib\.\w+|s3transfer\.S3Transfer\.upload_file|(?:\.)?utils\._http\.http_stream_backoff|ansible\.module_utils\.urls\.fetch_url)\s*\(/s }),
  call({ id: "python.exfiltration", category: "data_exfiltration", title: "Uploads or transmits data", severity: "high", confidence: "medium", message: "Uses an upload, POST, object-storage, confirmed socket send, or remote writer operation.", pattern: /^(?:requests\.(?:Session\.)?(?:post|put|patch)|httpx\.(?:(?:Async)?Client\.)?(?:post|put|patch)|boto3\.\w+|[\w.]+\.(?:upload_file|put_object)|socket\.socket\.(?:send|sendall)|(?:writer|stream_writer|remote_writer)\.write)\s*\(/s }),
  call({ id: "python.destructive", category: "destructive_behavior", title: "Deletes data recursively or writes a disk device", severity: "critical", confidence: "medium", message: "Performs recursive deletion or direct disk writes.", pattern: /^(?:shutil\.rmtree|os\.removedirs)\s*\(|(?:open|os\.open)\s*\(\s*["']\/dev\//s }),
...
export const JAVASCRIPT_RULES: LanguageRule[] = [
  ...
  jsCall({ id: "javascript.dynamic-code", category: "dynamic_execution", title: "Executes dynamic JavaScript or shell code", severity: "high", confidence: "high", message: "Uses dynamic code evaluation, VM, or a confirmed process execution package.", pattern: /(?:^|\.)(?:eval|Function|runIn\w+|compileFunction|exec|execSync|spawn|spawnSync|execa|execaCommand)\s*\(/s }),
