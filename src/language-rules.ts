...
export const PYTHON_RULES: LanguageRule[] = [
  ...
  call({ id: "python.dynamic-code", category: "dynamic_execution", title: "Executes dynamic Python or shell code", severity: "high", confidence: "high", message: "Uses a dynamic code or shell execution API.", pattern: /^(?:eval|exec|compile|os\.system|os\.popen|subprocess\.(?:run|call|Popen|check_call|check_output))\s*\(/s }),
  call({ id: "python.persistence", category: "persistence", title: "Modifies startup or scheduled execution", severity: "high", confidence: "medium", message: "Writes or installs a common persistence mechanism.", pattern: /(?:open|Path|write_text|write_bytes|shutil\.copy\w*)\s*\([^)]*(?:crontab|\/etc\/cron|systemd|LaunchAgents?|LaunchDaemons?|\.bashrc|\.profile|\.zshrc)/is }),
  call({ id: "python.credential-access", category: "credential_access", title: "Accesses credentials or secrets", severity: "high", confidence: "high", message: "Reads a common credential location, keychain, browser data, or environment secrets.", pattern: /(?:open|Path|read_text|read_bytes|os\.getenv|keyring\.\w+)\s*\([^)]*(?:\.ssh|\.aws[\/\\]credentials|\.config[\/\\]gcloud|Keychains?|Login Data|Cookies|TOKEN|SECRET|PASSWORD|KEY)/is }),
  ...
  call({ id: "python.system-modification", category: "system_modification", title: "Modifies sensitive system configuration", severity: "critical", confidence: "high", message: "Writes sensitive system, network, firewall, proxy, or certificate configuration.", pattern: /(?:open|Path|write_text|write_bytes|shutil\.(?:copy|move))\s*\([^)]*(?:\/etc\/|sudoers|resolv\.conf|iptables|nftables|certificates?|proxy)/is }),
  ...
  call({ id: "python.network", category: "network_egress", title: "Creates an outbound network connection", severity: "medium", confidence: "medium", message: "Uses an HTTP, URL, socket, SSH, or FTP client.", pattern: /^(?:requests\.(?:get|post|put|patch|request)|urllib\.request\.(?:urlopen|urlretrieve)|httpx\.\w+|socket\.(?:socket|create_connection)|paramiko\.\w+|ftplib\.\w+)\s*\(/s }),
  call({ id: "python.exfiltration", category: "data_exfiltration", title: "Uploads or transmits data", severity: "high", confidence: "medium", message: "Uses an upload, POST, object-storage, or socket send operation.", pattern: /^(?:requests\.(?:post|put|patch)|httpx\.(?:post|put|patch)|boto3\.\w+|[\w.]+\.(?:upload_file|put_object|send|sendall))\s*\(/s }),
...
export const JAVASCRIPT_RULES: LanguageRule[] = [
  ...
  jsCall({ id: "javascript.network", category: "network_egress", title: "Creates an outbound network connection", severity: "medium", confidence: "medium", message: "Uses HTTP, fetch, socket, SSH, DNS, or FTP networking.", pattern: /^(?:fetch|axios\.\w+|https?\.(?:get|request)|net\.(?:connect|createConnection)|tls\.connect|dns\.\w+|[\w.]+\.(?:connect))\s*\(/s }),
  ...
  jsCall({ id: "javascript.second-stage", category: "second_stage_payload", title: "Retrieves or unpacks a second-stage payload", severity: "high", confidence: "medium", message: "Downloads or extracts an archive that may contain another payload.", pattern: /^(?:(?:fetch|https?\.get|axios\.get)\s*\([^)]*\.(?:zip|tar|tgz|gz|xz)\b|(?:extract|unzip|tar\.\w+)\s*\()/is }),
];
